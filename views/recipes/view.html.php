...
$total_records = count($template->images);
...
$i = $started_record;
while($template->images AND $j<$records_per_page)
{
    $name = $template->images[$i]->image_name;
	...
	//echo "<table>";
	//echo "<tr><td>";
	echo "<img src='".$src."'width='500' height='500'></td><td>";
	...
	{
		if ($_SESSION['user']['id'] == $template->recipe->user_id)
		{
			if ($template->images[$i]->is_cover == "no")
			{
				echo "<a href='/myrecipe/recipes/" . $template->images[$i]->recipe_id . "/" . $template->images[$i]->image_name . "'>Set to cover</a>";
			}
			elseif ($template->images[$i]->is_cover == "yes")
			{
				echo "This is the cover of this recipe!";
			}
		}
	}
